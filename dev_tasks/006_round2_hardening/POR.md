...
Branch `dev/006-round2-hardening` (latest commit `ee73fc8`). Stage 0
(version routing) is complete — see Progress section below.

Key files:
- `src/synthgen/mcp/tools/compat.py` — version-aware code-gen helpers (Stage 0, new)
...
- `addon/synthgen_mcp/server.py` — MCP server, `/health`, dynamic instructions
- `addon/synthgen_mcp/executor.py` — transport, `MainThreadExecutor`, version detection
- `tests/test_version_routing.py` — version routing tests (Stage 0, new)
- `tests/test_mutation_hardening.py` — existing tests

296 tests passing as of Stage 0 completion.
...
**✅ FIXED in Stage 0.** The broken `dir()` + `getattr()` + `.default_value`
pattern has been replaced with `emit_iter_inputs()` from `compat.py`. The
generated code now uses correct bracket access on 5.x and `mod[ident]` on
4.x. Also surfaces `attribute_name` to distinguish literal vs attribute-driven
inputs.

**Remaining:** Live validation with a modifier that has exposed parameters
on Blender 5.2 (deploy + run `get_modifier_inputs` and confirm non-empty
`inputs` list).
...
**✅ FIXED in Stage 0.** Both `set_parameter` and `sweep` now use
`emit_write_input()` from `compat.py`. The broken `getattr` +
`.default_value` pattern has been replaced with correct bracket access
(`mod.properties.inputs[ident]['value'] = v`) on 5.x and `mod[ident] = v`
on 4.x. No runtime version branching — code-gen time only.

Unit tests confirm bracket access for 5.x and id-property for 4.x in both
`test_version_routing.py` and updated `test_mutation_hardening.py`.

**Remaining:** Live validation — set a parameter on Blender 5.2 and read
it back via the fixed `get_modifier_inputs` to confirm the round-trip.

**WARNING (still applies):** Do NOT use `bl_system_properties_get()` — it
hangs Blender's main thread with no exception, and recovery requires
force-quit.
...
**✅ FIXED in Stage 0.** Docstring now lists both IDs:
`"BLENDER_EEVEE" (5.x) / "BLENDER_EEVEE_NEXT" (4.x)`. The `eevee_engine_id()`
helper in `compat.py` is available if version-aware validation is needed later.
...
| Feedback | Stage | Step | File | Status |
|---|---|---|---|---|
| (infrastructure) | 0 | 0.1–0.4 | `server.py`, `compat.py`, all tools | ✅ done |
| A1 — links before params | 3 | 3.1 | `blender.py` | TODO |
| A2 — no rollback | 3 | 3.2 | `blender.py` | TODO |
| A3 — can't see existing nodes | 3 | 3.3 | `blender.py` | TODO |
| B — viewport never redraws | 1 | 1.1 | `blender.py` | TODO |
| C — evaluate_object mesh-only | 2 | 2.1 | `verify.py` | TODO |
| D — get_modifier_inputs empty | 2 | 2.2 | `verify.py` | ✅ done (Stage 0) |
| E — set_parameter broken 5.x | 4 | 4.1 | `pipeline.py` | ✅ done (Stage 0) |
| F — int coercion min/max | 5 | 5.1 | `blender.py` | TODO |
| G — disabled sockets listed | 5 | 5.2 | `blender.py` | TODO |
| H — modifier default diverges | 5 | 5.3 | `blender.py` | TODO |
| (EEVEE docstring) | 5 | 5.4 | `blender.py` | ✅ done (Stage 0) |

## 005 fixes that needed correction — ✅ RESOLVED in Stage 0

Three tools from dev_task 005 shipped with the **wrong Blender 5.x API access
pattern**. All three were fixed in Stage 0 by migrating to `compat.py` helpers:

| Tool | 005 code (broken) | Correct 5.2 API | Fixed in |
|---|---|---|---|
| `get_modifier_inputs` | `getattr(mod.properties.inputs, name).default_value` | `mod.properties.inputs[ident]['value']` | Stage 0.4 |
| `set_parameter` | `getattr(mod.properties.inputs, sid).default_value = v` | `mod.properties.inputs[sid]['value'] = v` | Stage 0.4 |
| `sweep()` inner loop | same `getattr` pattern | same bracket pattern | Stage 0.4 |

The `dir()` + `getattr()` approach returned nothing because `properties.inputs`
members are not Python attributes — they are RNA properties accessed via
bracket/subscription syntax. The compat helpers now route to the correct
access pattern based on the cached Blender version at code-gen time.
